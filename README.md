...
* What is a [*obstruction-free*](https://en.wikipedia.org/wiki/Non-blocking_algorithm#Obstruction-freedom) (at any point, a single thread executed in isolation (i.e., with all obstructing threads suspended) for a bounded number of steps will complete its operation) > [*lock-free*](https://en.wikipedia.org/wiki/Non-blocking_algorithm#Lock-freedom) (when the program threads are run for a sufficiently long time, at least one of the threads makes progress (for some sensible definition of progress)) > [*wait-free*](https://en.wikipedia.org/wiki/Non-blocking_algorithm#Wait-freedom) (every operation has a bound on the number of steps the algorithm will take before the operation completes) > algorithm?
